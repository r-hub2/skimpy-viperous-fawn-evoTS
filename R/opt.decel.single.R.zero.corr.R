...
#'
#' @details The function searches - using an optimization routine - for the maximum-likelihood solution for a multivariate Unbiased Random Walk model with uncorrelated trait changes and with decreasing (exponential decelerating) rate of change through time.
#'
...
# via -(abs(r)*...)), and data-SD-scaled additive jitter for the ancestral
# values. Result is also clamped to the lower/upper bounds (NA bounds treated
# as -Inf/Inf) so restarts can't violate constraints.
