#' @title Fit multivariate Unbiased Random Walk with decreasing (exponential decaying) rate of change through time.
#'
#' @description Function to find maximum likelihood solution to a multivariate Unbiased Random Walk model with decreasing (exponential decaying) rate of change through time.
#'
...
#'
#' @details The function searches - using an optimization routine - for the maximum-likelihood solution for a multivariate Unbiased Random Walk model with decreasing (exponential decaying) rate of change through time.
#'
...
opt.decel.single.R<-function (yy, method="L-BFGS-B", hess = FALSE, pool = TRUE, trace=FALSE, iterations=NULL, iter.sd=NULL)
{
...
### v.1.4 ###
# V.1.3 perturbed the whole parameter vector identically, then
...
# log-normal jitter for the trait-variance parameters and for |r| (kept negative
# via -(abs(r)*...)), small additive jitter for the covariance terms, and
# data-SD-scaled additive jitter for the ancestral values. Result is also
# clamped to the lower/upper bounds (NA bounds treated as -Inf/Inf) so restarts
