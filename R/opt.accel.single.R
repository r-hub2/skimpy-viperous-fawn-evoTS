#' @title Fit multivariate Unbiased Random Walk with increasing (exponential accelerating) rate of change through time.
#'
#' @description Function to find maximum likelihood solution to a multivariate Unbiased Random Walk model with increasing (exponential accelerating) rate of change through time.
#'
...
#'
#' @details The function searches - using an optimization routine - for the maximum-likelihood solution for a multivariate Unbiased Random Walk model with increasing (exponential accelerating) rate of change through time.
#'
...
opt.accel.single.R<-function (yy, method="L-BFGS-B", hess = FALSE, pool = TRUE, trace=FALSE, iterations=NULL, iter.sd=NULL)
{
...
# every optimizer call.
  ta     <- outer(yy$tt[,1], yy$tt[,1], FUN = pmin)  # pre-compute time-distance matrix
...
# log-normal jitter for the trait-variance parameters and for r (keeps both
# positive), small additive jitter for the covariance terms, and data-SD-scaled
# additive jitter for the ancestral values. Result is also clamped to the
# lower/upper bounds (NA bounds treated as -Inf/Inf) so restarts can't violate
# constraints.
...
# passing to optim().
        www[[k]]<-optim(init.par, fn = logL.joint.accel.decel.single.R, y = y, m = m, n = n, anc.values = anc.values, ta = ta, se_vec = se_vec,
                        control = list(fnscale = -1, maxit=10000, trace = trace), method = "L-BFGS-B", hessian = hess, lower = replace(lower.limit, is.na(lower.limit), -Inf), upper = replace(upper.limit, is.na(upper.limit), Inf))
      }
...
# passing to optim().
        www[[k]]<-optim(init.par, fn = logL.joint.accel.decel.single.R, y = y, m = m, n = n, anc.values = anc.values, ta = ta, se_vec = se_vec,
                        control = list(fnscale = -1, maxit=10000, trace = trace), method = "SANN" , hessian = hess, lower = replace(lower.limit, is.na(lower.limit), -Inf), upper = replace(upper.limit, is.na(upper.limit), Inf))
      }
...
# the `lower`/`upper` bounds now replace NA entries with -Inf/Inf.
  ...
  ancestral.values<-w$par[(length(init.trait.var) + length(init.cov.traits) +1) : (length(init.par)-1)]

  r<-w$par[length(init.par)]

  K<-length(w$par)

  wc<-as.evoTS.multi.BW.acceldecel.fit(converge, modelName = "Multivariate model: Random walk with accelerating rate of evoluton (R matrix with off-diagonal elements)", logL = w$value, ancestral.values = ancestral.values, SE.anc = SE.anc, r = r, SE.r = SE.r, R = R, SE.R = SE.R,
